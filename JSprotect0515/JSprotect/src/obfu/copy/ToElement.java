...
class ToElement{
    ...
    private void DealSpeAssList(){
        for(int i=0;i<SpeAssList.size()&&SpeAssList.get(i) instanceof Assignment;i++){
            ...
            if(Right instanceof InfixExpression||Right instanceof ConditionalExpression){
    ...
    private ArrayList<String> Split6(String str){
        ArrayList<String> Str=new ArrayList<String>();
        if(str.length()>=4){
            String str1=str.substring(0,(int)(str.length()/6));
            String str2=str.substring((int)(str.length()/6),(int)((str.length()/6)*2));
            String str3=str.substring((int)((str.length()/6)*2),(int)((str.length()/6)*3));
            String str4=str.substring((int)((str.length()/6)*3),(int)((str.length()/6)*4));
            String str5=str.substring((int)((str.length()/6)*4),(int)((str.length()/6)*5));
            String str6=str.substring((int)((str.length()/6)*5),str.length());
            Str.add(str1);
            Str.add(str2);
            Str.add(str3);
            Str.add(str4);
            ...
            if(Init instanceof StringLiteral){
                ...
                if(str!=null){
                    ...
                    if(str.size()<3){
                        ...
                        Name FunName6=new Name();
                        FunName6.setIdentifier(NislFunction.get(2));
                        FunctionCall join6=new FunctionCall();
                        join6.setTarget((AstNode)FunName6.clone());
                        List<AstNode> Argu=new ArrayList<AstNode>();
                        for(int k=0;k<str.size();k++){
							/*Name argu=new Name();
							argu.setIdentifier(NodeName.get(k));
                            ...
                            string.setQuoteCharacter('"');
                            Argu.add(string);
                            string.setParent(join6);
                            string.setRelative(join6.getPosition());
                        }
                        join6.setArguments(Argu);
                        NewInit=(AstNode)join6.clone();
                    }
